...
--fair algorithm IdGenerator {
  ...
  process(id \in 1 .. NumberOfProcesses) {
   ...
   if (Locking) {
    ...
    \* retrieve the value of field lastId from the this and store it back on the top of the stacks
    getfield_lastId: 
    ...
    \* duplicate the value on top of the stack and put it before   
    dup_x1: 
...
id(self) == checkLocking(self) \/ waitForLock(self) \/ aload0(self)
               ...
               \/ iadd(self) \/ dup_x1(self) \/ putfield(self) \/ ret(self)
               \/ unlock(self)
...
\* Last modified Sun Dec 13 12:49:35 EST 2015 by balopat
